\### 3.png



| Шаг | Роль |

|-----|------|

| 1. Создание запроса | Инициатор |

| 2. Внесение технологии в тех. стек в статусе "Consideration" | Координатор |

| 3. Оценка по техническим критериям | Инициатор |

| 4. Уточнение требований к технологии | Совет по технологиям, Координатор, Владелец продукта |

| 5. Оценка ограничений и рисков принятия новой мажорной версии | Координатор |

| 6. Презентация новой мажорной версии | Инициатор |

| 7. Принятие решения | Главный корпоративный архитектор, Совет по технологиям |

| 8. Внесение записи в тех. стек, оповещение | Координатор |

| 9. Перевод прошлой мажорной версии в статус Deprecated | Координатор |

| 10. Оповещение | Координатор |

| 11. Планирование обновления продукта, в рамках которого поставляется технология | Координатор |

| 12. Планирование автоматизации ресурсов на стороне ДИТ | Координатор |

| 13. Изменение атрибутов технологии в технологическом стеке | Координатор |



\### 6.png



| Шаг | Роль |

|-----|------|

| 1. Создание запроса | Инициатор |

| 2. Подтверждение потребности в отказе, изменении условий использования | Координатор |

| 3. Определение пользователей технологии | Совет по технологиям |

| 4. Оценка возможностей и трудозатрат для изменения условий использования технологии | Совет по технологиям |

| 5. Принятие решения | Главный корпоративный архитектор, Совет по технологиям |

| 6. Изменение статуса технологии, Информирование | Координатор |

| 7. Оповещение клиентов | Команда разработки |

| 8. Выполнение работ для отказа/изменения условий использования технологии | ДЭиС |



\### 9.png



| Шаг |

|-----|

| 1. Создание запроса |

| 2. Рассмотрение запроса |

| 3. Анализ запроса |

| 4. Принятие решения о реализации изменения |

| 5. Реализация улучшений и исправление несоответствий |

| 6. Оценка результатов |



\### 45.png



| Шаг |

|-----|

| 1. Выполнить сценарий выбора СФЛ при назначении роли |

| 2. Выдать сообщение о невозможности совмещения ролей |

| 3. Сохранить скан документа, удостоверяющего полномочия |

| 4. Выбрать все нужные полномочия из списка |

| 5. Запросить информацию о праве передоверия |

| 6. Запросить срок действия документа, удостоверяющего полномочия |

| 7. Запросить прочие атрибуты роли "Доверенное лицо" |

| 8. Запросить все обязательные атрибуты роли |



\### 48.png



| Шаг |

|-----|

| 1. Выдать информацию о всех ролях закрываемого СФЛ |

| 2. Удалить все назначенные роли |

| 3. Установить признаки и причину закрытия СФЛ |



\### 53.png



| Шаг |

|-----|

| 1. Маршрутизатор на ответственного |

| 2. Задача 1 |

| 3. Задача 2 |

| 4. Маршрутизатор на ответственного |

| 5. Задача 3 |



\### 60.png



| Шаг |

|-----|

| 1. Получаем тикет из Jira |

| 2. Вытаскиваем заголовок, продукт, описание |

| 3. Ищем похожие тикеты |

| 4. Ищем TOP3 тикетов |

| 5. Добавляем ссылки в первоначальный тикет |

| 6. Пытаемся определить причину и процесс поиска решения в прилинкованных тикетах |

| 7. На основании информации даем ответ в комментариях |



\### 65.png



| Шаг |

|-----|

| 1. Опросник |

| 2. Место силы вся информация по всем ПП |

| 3. Tools |

| 4. PSC и другое ПО |



\### 72.png



| Шаг |

|-----|

| 1. Начало |

| 2. Наполнить чайник |

| 3. Включить чайник |

| 4. Ждать закипания |

| 5. Событие: Чайник закипел |

| 6. Таймер: 30 сек |

| 7. Автовыключение |

| 8. Действие: выключить вручную |

| 9. Чай готов |



\### 135.png



| Шаг |

|-----|

| 1. Конфликт/несогласие |

| 2. Попытка переговоров |

| 3. Обращение к медиатору/арбитру |

| 4. Арбитражный процесс |

| 5. Решение арбитра |

| 6. Коррекция договора |

| 7. Завершено: Действует |

| 8. Оформление расторжения по решению |

| 9. Завершено: Расторжение |



\### 149.png



| Шаг |

|-----|

| 1. Зачисление |

| 2. Обновить резюме |

| 3. Искать гибкую работу |

| 4. Найдена? |

| 5. Договориться о графике |

| 6. Совмещение работы и учебы |

| 7. Фокус на учебе |

| 8. Только учеба |

